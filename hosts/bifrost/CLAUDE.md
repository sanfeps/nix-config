...
- **`dns.nix`** — AdGuard Home. Bound to `0.0.0.0:53`, DoH upstream Quad9. WebUI on `127.0.0.1:3000`, reverse-proxied by Caddy at `https://adguard.lan.valgrindr.net`. LAN-zone rewrites point each `*.lan.valgrindr.net` name at the host that terminates TLS for it: bifrost-local services → `192.168.1.55`, asgard apps (Immich, Ghostfolio, Home Assistant, Firefly, and the media stack) → `192.168.1.54` (their own Caddy). **Exception: `fluxer.lan.valgrindr.net` → `100.64.0.2`** (asgard's *tailnet* IP), so it lands on asgard's Fluxer-only tailnet listener — that's how `group:guest` reaches Fluxer without seeing asgard's other apps (see `hosts/asgard/services/fluxer/CLAUDE.md`). `mutableSettings = false`: changes only via Nix.
