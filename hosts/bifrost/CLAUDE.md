...
- **`media-proxies.nix`** — Caddy handles for the media stack on asgard. Six `*.lan.valgrindr.net` names (jellyfin, seerr, qbittorrent, prowlarr, sonarr, radarr) reverse-proxy to `192.168.1.54:<port>` over the wildcard cert. Until asgard's `./media` import is activated (waiting on `media/mullvad-wg-conf` in sops), these handles 502 cosmetically — that's the scaffold-mode contract.
