...
- **`dns.nix`** — AdGuard Home. Bound to `0.0.0.0:53`, DoH upstream Quad9. WebUI on `127.0.0.1:3000`, reverse-proxied by Caddy at `http://adguard.lan.valgrindr.net`. LAN-zone rewrites point every `*.lan.valgrindr.net` name at bifrost (`192.168.1.55`), except `firefly.lan.valgrindr.net` which points at asgard (`192.168.1.54`) — Firefly's PHP-FPM Unix socket can't be proxied across hosts. `mutableSettings = false`: changes only via Nix.
- **`caddy.nix`** — Single Caddy instance with Njalla DNS-01 plugin (built via `pkgs.caddy.withPlugins`). Two top-level vhosts:
  ...
  - `*.lan.valgrindr.net` (wildcard, LE via DNS-01 with `acme_dns njalla {env.NJALLA_API_TOKEN}`): per-service routing via `@host` matchers + `handle` blocks. Adguard local (127.0.0.1:3000); Ghostfolio (192.168.1.54:3333); Home Assistant (192.168.1.54:8123). Fallback `respond "bifrost edge - unknown subdomain" 404`.
