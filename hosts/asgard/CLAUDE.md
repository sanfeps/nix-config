...
- **`postgresql/`** — the **shared PostgreSQL instance** (`server.nix`) plus the instance-wide dump timer (`backups.nix`). Lives at the top of `services/`, not under `finances/`, because it is host infrastructure: several unrelated app domains keep their databases on this one server. Listens on `127.0.0.1` only (TCP enabled so containers can connect via `--network=host`).
  - **Backups**: `pg_dump --format=custom` daily (`OnCalendar=daily`, `Persistent=true`, 30min jitter), 90-day rotation, into `/persist/var/backups/postgres/<db>/`. Validated end-to-end (restorable). It enumerates **every** non-template database via `pg_database` — there is no allowlist, so a new app's DB is picked up automatically the first time the timer fires. This is a **local** copy only; see `yamtrack.nix` for the per-app NAS leg.
  - Isolation between apps is per-database + per-role (each app owns its own DB and role). The cost of sharing is **version coupling** — a major-version bump moves every app at once.
  - **GOTCHA — order role-password oneshots after `postgresql-setup.service`, not `postgresql.service`.** nixpkgs runs `ensureDatabases`/`ensureUsers` in a *separate* `postgresql-setup.service`, so a `sops`-driven `ALTER USER … PASSWORD` unit that only declares `After=postgresql.service` races role creation and dies with `ERROR: role "<app>" does not exist`. The app then loops on `FATAL: password authentication failed`, because its env file has a password Postgres never applied. Bit yamtrack for real on 2026-08-12; ghostfolio had the same wrong ordering but survived because its role predated the unit. Both fixed — copy the fixed ordering for any new app.
- **`finances/`** — per-app finance modules (databases live on the shared instance above):
...
- **`yamtrack.nix`** — **Yamtrack** at `https://yamtrack.lan.valgrindr.net`: self-hosted tracker for films, books, TV and games (what was watched/read + ratings). Podman container (`modules/nixos/services/containers/yamtrack.nix`, image `ghcr.io/fuzzygrim/yamtrack`) on `--network=host`, own `yamtrack` database + role on the shared Postgres, own Redis on `127.0.0.1:6380` (6379 is Ghostfolio's). Fronted by asgard's local Caddy; AdGuard answers `192.168.1.54`.
  - **It binds `0.0.0.0:8000`, not loopback** — the image hardcodes `listen 8000` in its bundled nginx and exposes no bind knob. Privacy comes from the firewall (8000 is not in `allowedTCPPorts`), exactly like Jellyfin on draupnir. Don't "fix" this by opening the port.
  - **Jellyfin integration is a webhook PUSH**, which is why the app doesn't need to live next to Jellyfin: Jellyfin's Webhook plugin POSTs `Play`/`Stop`/`MarkPlayed`/`MarkUnplayed` to `/webhook/jellyfin/<user-token>` (`login_not_required`, `csrf_exempt`). Installing that plugin is a **manual UI step on draupnir** — the one non-declarative part. Books are always manual entry.
  - `URLS` must match the public origin or logins fail with CSRF errors rather than anything obvious. `REGISTRATION=False` — single-user.
  - Backups live in `appdata-backup.nix`, not here.
  - **KNOWN UPSTREAM BUG — webhooks 500 on a failed anime-mapping fetch (2026-08-12).** Every webhook (movies included, not just anime) calls `integrations/webhooks/anime_mappings.py::fetch_mapping_data`, which downloads `github.com/anibridge/anibridge-mappings/releases/download/v3/mappings.min.json`. Yamtrack lets that failure propagate, so the webhook returns **500 and the play event is lost** (Jellyfin does not retry). On asgard that fetch fails **reliably** through Yamtrack's shared `LimiterSession` (`app/providers/services.py`) with `RemoteDisconnected('Remote end closed connection without response')` ×3 — while a plain `requests.get` to the *same URL, in the same container, in the same process* returns 200. Ruled out: DNS (IPv4-only, `140.82.121.4`, identical inside and outside the container), IPv6 (no AAAA), routing, TLS, and the netns (yamtrack is on host networking).
    **Workaround (imperative, in Redis — survives container restarts, NOT a rebuild):**
    ```
    sudo podman exec yamtrack python manage.py shell -c "import requests; from django.core.cache import cache; from integrations.webhooks.anime_mappings import CACHE_KEY, URL; cache.set(CACHE_KEY, requests.get(URL, timeout=60).json(), None); print('cached')"
    ```
    `timeout=None` ⇒ never expires, so the webhook path finds it in cache and never touches GitHub. **If webhooks start returning 500 again, re-run that command first.** Verify with a synthetic POST of a movie payload to `/webhook/jellyfin/<token>`; expect `200`.
- **`appdata-backup.nix`** — ships the app data that is **not re-derivable** to the NAS. Daily timer at 03:30 (after `postgres-backup`, which is `daily` + ≤30min jitter, so it copies the current night's dump).
  - **The CSV export is the primary artifact**, the pg_dump is the second copy. Yamtrack has no management command for exporting, so the job calls the same function the UI's download button calls — `integrations.exports.generate_rows` — in-process via `podman exec yamtrack python manage.py shell -c` (Django 5.2, so `-c` exists). That makes the file byte-identical to a UI export and re-importable by Yamtrack, with no session cookie or HTTP hop. One CSV per active user, written to `/var/lib/yamtrack/exports` (the module's `exportDir`, bind-mounted at `/exports`).
  - **Allowlisted, unlike the dump timer.** `postgresql/backups.nix` dumps *every* database locally; only the databases in this file's `databases` list leave the box (currently just `yamtrack`). Add `firefly-iii` here when the finance offsite is built.
  - **Stable filenames, no rotation logic.** Each run overwrites `<nas>/yamtrack/yamtrack-<user>.csv` and `<nas>/postgres/<db>.dump`. History comes from draupnir's sanoid snapshots of `tank/appdata`, so yesterday's copy is `/tank/appdata/.zfs/snapshot/<snap>/…`. All writes are write-then-rename so a snapshot can never catch a half-written file.
  - **Mount guard is load-bearing.** `/mnt/nas/appdata` is an automount; the script refuses to run if `mountpoint -q` fails. Without it, a draupnir outage would silently write backups to asgard's local disk — the same path that filled the Proxmox thin-pool and froze the VM into `io-error` (2026-07-27).
  - Runs as **root** (needs the podman socket and read access to `/var/backups/postgres`, mode `0700 postgres`). draupnir's export squashes that root to its local `appdata` uid/gid 1501 — see `hosts/draupnir/services/nfs.nix`.
  - **Not yet wired: failure alerting.** A silently failing backup is the classic trap; asgard has no ntfy publish token yet (bifrost hosts the bus). Add an `OnFailure` unit publishing to ntfy when that token exists.
