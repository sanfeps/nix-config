...
`services/caddy.nix` pins `default_bind 192.168.1.54`, so every vhost binds the **LAN IP only** — except Fluxer, which adds `bind 192.168.1.54 100.64.0.2` to also listen on asgard's **tailnet IP**. That makes `100.64.0.2:443` a Fluxer-only listener so `group:guest` tailnet users (granted `asgard:443`) reach Fluxer and nothing else; the other apps stay on the LAN IP (reachable on-LAN, and off-LAN to admins via bifrost's `192.168.1.0/24` subnet route). `ip_nonlocal_bind=1` lets Caddy bind the tailnet IP without waiting on tailscaled. See `services/fluxer/CLAUDE.md`.
