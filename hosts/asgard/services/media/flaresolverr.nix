...
in {
  ...
  # NB: the /etc/resolv.conf bind that makes Chromium resolve inside the netns
  # is applied centrally to every confined unit in vpn.nix (it turned out the
  # glibc *arrs need it too, not just Chromium). See the comment there.
}
