...
**`single-request-reopen` (intermittent `EAI_AGAIN`).** With a *single* tunnel-routed resolver in a dual-stack netns, glibc fires the A and AAAA queries in parallel from one socket; over the WireGuard tunnel one reply intermittently gets dropped, so `getaddrinfo` returns `EAI_AGAIN` and every confined .NET app surfaces it as `SocketException (11): Resource temporarily unavailable (<host>:443)` — Prowlarr indexer tests fail, Radarr's SkyHook metadata lookups 503, and a Seerr request that hits the 503 gets marked **FAILED** (the request itself is fine — re-request and it goes through). `vpn.nix` appends `options single-request-reopen` to `/etc/netns/mullvad/resolv.conf` via an `ExecStartPost` on `mullvad.service` (re-applied every (re)start because `mullvad-up` does `rm -rf /etc/netns/mullvad` first), forcing the two lookups to run sequentially on separate sockets. We can't add a fallback nameserver — the netns firewall drops UDP/53 to anything but the configured resolver.

**The actual root cause was IPv6 connects, not DNS caching (2026-08-11).** The recurring `SocketException(11): Resource temporarily unavailable (<host>:443)` was — per the stack trace — at *connect* (`ManagedHttpDispatcher.attemptConnection(AddressFamily)`), **after** a successful resolve, not a DNS `EAI_AGAIN`. systemd-resolved (see next paragraph) was handing the *arrs AAAA records; the IPv6 path over the WireGuard tunnel flakes, the connect fails with EAGAIN, and .NET poisons its pool → every Seerr request instant-FAILED until restart. Direct proof: `dig @10.64.0.1 <host>` from the netns answers **instantly with IPv4**, and `curl -4`/`curl -6` both 404 — i.e. resolution and IPv4 connects are fine; IPv6 is the flaky bit. **Fix (two parts, both in `vpn.nix` ExecStartPost):** (1) resolv.conf → `nameserver 10.64.0.1` + `options single-request-reopen` (the direct in-tunnel resolver, reliable); (2) **disable IPv6 in the netns** — `ip netns exec mullvad sysctl -w net.ipv6.conf.{all,default}.disable_ipv6=1` — so `getaddrinfo` (AI_ADDRCONFIG) returns IPv4-only and the *arrs never attempt the flaky IPv6 connect. Combined with the nsswitch fix below, DNS also stops leaking out the host. **A caching dnsmasq was tried first and dropped:** it listened on `127.0.0.1:53` in the netns but silently would not forward to `10.64.0.1` from inside the netns (`dig @127.0.0.1` timed out, `queries sent 0` to upstream), and the direct resolver is reliable so caching bought nothing — don't reintroduce it without solving the in-netns forward.

**nss-resolve was silently defeating all of the above (2026-08-11).** The nixpkgs bump made systemd-resolved the nss primary: `/etc/nsswitch.conf` became `hosts: mymachines resolve [!UNAVAIL=return] files myhostname dns`. `nss-resolve` reaches systemd-resolved over its unix socket **even from inside the netns**, so it answered every lookup on the HOST network with `[!UNAVAIL=return]` — the `dns` module (our resolv.conf → 10.64.0.1) was **never consulted**. Two consequences: the *arrs got AAAA and tried flaky IPv6 connects over the tunnel (→ the `SocketException(11)` above), **and confined DNS leaked out the host instead of the tunnel**. Diagnosis tell: `sudo cat /proc/$(pgrep -x Radarr)/root/etc/resolv.conf` shows `127.0.0.53` and `ip netns exec mullvad getent ahosts <h>` returns IPv6. Fix: `vpn.nix` binds a custom `/etc/nsswitch.conf` (`nsswitchConf`, `hosts: files myhostname dns` — no `resolve`/`mymachines`) over each confined service, so hosts resolve strictly via `dns` → the netns resolv.conf (10.64.0.1, over the tunnel). **The resolv.conf bind is inert without this.**
...
**Durable fix (2026-08-11) — two parts in `vpn.nix`:**

1. **`partOf = ["mullvad.service"]` on all four confined units** (in the same `genAttrs` block as the resolv.conf bind). VPN-Confinement only sets `BindsTo`+`After`, which stops them when mullvad stops but does NOT recycle them when mullvad *restarts* — so netns churn left the .NET *arrs holding a poisoned SocketsHttpHandler pool. `PartOf` propagates mullvad's restart to them; `After=mullvad` orders them behind the resolv.conf write, so a deploy or `systemctl restart mullvad` now recycles them clean.

2. **Warm-tunnel readiness gate on `mullvad.service`** (a second `ExecStartPost`). VPN-Confinement flips the unit active as soon as the interface is up, but the WireGuard handshake lags a few seconds, so dependents started against a *cold* tunnel and cached the failure anyway (PartOf recycled them, but into the same cold window). The gate loops `ip netns exec mullvad dig +short @10.64.0.1 api.radarr.video` (bypasses nss, probes the exact tunnel+resolver path; netns firewall permits UDP/53 to that resolver) for ≤30s before letting mullvad go active — so `After=mullvad` now genuinely means "tunnel usable". This is what actually kills the deploy-time "whole chain breaks (Byparr `camoufox InvalidIP: Failed to get IP`, indexers fail even non-CF ones)" symptom. Bounded + `exit 0` on timeout so a genuinely-down tunnel doesn't wedge activation.

The manual `systemctl restart podman-byparr` / `systemctl restart <arr>` is now only a fallback. **Note the manual "delete + re-add the indexer in Prowlarr" workaround was a red herring** — by the time you did it the tunnel had warmed; a plain restart (or just waiting) recovers the same. Symptom (1) cured: deleting a Seerr request hung because Radarr 503'd its SkyHook lookup (`api.radarr.video` EAI_AGAIN) → Seerr logged `[Radarr] Failed to remove movie: Movie not found`.
