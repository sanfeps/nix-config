...
**In-netns caching dnsmasq (the durable fix, 2026-08-11).** `single-request-reopen` + the warm-tunnel gate reduced but never killed this — a genuinely dropped packet on the single lossy tunnel resolver still poisons the .NET pool, and it recurred constantly (Prowlarr indexers, Radarr SkyHook, Seerr FAILED). Fix: `vpn.nix` runs a **`mullvad-dnsmasq`** service confined to the netns (dnsmasq 2.93, `-k`, config in `dnsmasqConf`) listening on `127.0.0.1:53`, forwarding to `10.64.0.1` (`no-resolv` keeps its upstream fixed), with `cache-size=1000` + **`use-stale-cache=86400`** (serves an expired answer when the upstream momentarily can't be reached — covers the exact transient) + **`filter-AAAA`** (IPv4-only, what servarr's own error recommends). The `ExecStartPost` now **rewrites** `resolv.conf` to `nameserver 127.0.0.1` + `options single-request-reopen no-aaaa`, so the confined services resolve via cache instead of racing the tunnel on every lookup. Ordering: `mullvad` (warm-tunnel gate) → `mullvad-dnsmasq` (partOf+after mullvad) → the *arrs (`after`+`wants` mullvad-dnsmasq, so they never start against a dead resolver). dnsmasq binds the privileged `:53` via ambient `CAP_NET_BIND_SERVICE` under `DynamicUser`. If DNS still misbehaves, check `systemctl status mullvad-dnsmasq` and `journalctl -u mullvad-dnsmasq` first.

**nss-resolve was silently defeating all of the above (2026-08-11).** The nixpkgs bump made systemd-resolved the nss primary: `/etc/nsswitch.conf` became `hosts: mymachines resolve [!UNAVAIL=return] files myhostname dns`. `nss-resolve` reaches systemd-resolved over its unix socket **even from inside the netns**, so it answered every lookup on the HOST network with `[!UNAVAIL=return]` — the `dns` module (our resolv.conf → dnsmasq) was **never consulted**. Two consequences: dnsmasq/`filter-AAAA` were bypassed (the *arrs got AAAA and tried flaky IPv6 connects over the tunnel → `SocketException(11)` at *connect*, not DNS → pool poisoned → Seerr FAILED), **and confined DNS leaked out the host instead of the tunnel**. Diagnosis tell: `sudo cat /proc/$(pgrep -x Radarr)/root/etc/resolv.conf` shows `127.0.0.53`, and `ip netns exec mullvad getent ahosts <h>` returns IPv6 despite `filter-AAAA`. Fix: `vpn.nix` binds a custom `/etc/nsswitch.conf` (`nsswitchConf`, `hosts: files myhostname dns` — no `resolve`/`mymachines`) over each confined service, so hosts resolve strictly via dnsmasq. **The resolv.conf bind is inert without this.**
