...
- `services/immich/` — Immich photo/video library (the canary). `homelab.services.immich.{enable,url,host,port,mediaLocation,machineLearning}`. Owns the upstream `services.immich`, the local-Caddy vhost, and `/var/lib/immich` persistence. The storage backing for `mediaLocation` is host-specific — currently draupnir (`hosts/draupnir/services/immich.nix`, library on `/tank/immich`).
